...
//! use gategen2::generic_array::typenum::*;
//! use gategen2::intexpr::*;
//! use gategen2::*;
//! use gatesim::*;
//!
...
pub struct IntExprNode<T: VarLit + Debug, N: ArrayLength, const SIGN: bool> {
    pub(super) creator: Rc<RefCell<ExprCreator<T>>>,
    pub(super) indexes: GenericArray<usize, N>,
}

impl<T, N: ArrayLength, const SIGN: bool> IntExprNode<T, N, SIGN>
where
    ...
    <isize as TryFrom<T>>::Error: Debug,
{
    ...
    pub fn from_boolexprs(iter: impl IntoIterator<Item = BoolExprNode<T>>) -> Option<Self> {
        ...
        GenericArray::try_from_iter(iter.into_iter().map(|x| {
    ...
    pub fn filled(creator: Rc<RefCell<ExprCreator<T>>>, v: impl Into<Literal<T>>) -> Self {
        IntExprNode {
            ...
            indexes: GenericArray::try_from_iter(
    ...
    pub fn filled_expr(v: BoolExprNode<T>) -> Self {
        IntExprNode {
            ...
            indexes: GenericArray::try_from_iter(iter::repeat(v.index).take(N::USIZE)).unwrap(),
        }
    }
...
impl<T, N: ArrayLength> IntExprNode<T, N, false>
where
    T: VarLit + Neg<Output = T> + Debug,
    isize: TryFrom<T>,
    <T as TryInto<usize>>::Error: Debug,
    ...
    <isize as TryFrom<T>>::Error: Debug,
{
        ...
        N2: ArrayLength,
    {
        IntExprNode {
            ...
            indexes: GenericArray::try_from_slice(&self.indexes[start..start + N2::USIZE])
                .unwrap()
                .clone(),
        }
    }
    ...
    {
        GenericArray::try_from_iter(iter.into_iter().map(|x| self.indexes[x]))
            .map(|indexes| IntExprNode {
                creator: self.creator.clone(),
                indexes,
            })
            ...
            $ty2: ArrayLength,
        {
            ...
            fn try_from(v: IntExprNode<T, $ty2, SIGN2>) -> Result<Self, Self::Error> {
                ...
                Ok(IntExprNode::<T, $ty1, false>{ creator: v.creator.clone(),
                    indexes: GenericArray::try_from_slice(&v.indexes[0..len1]).unwrap().clone() })
            }
        }
...
impl<T: VarLit, N: ArrayLength> TryFrom<IntExprNode<T, N, false>> for IntExprNode<T, N, true> {
    type Error = IntError;
...
impl<T: VarLit, N: ArrayLength> TryFrom<IntExprNode<T, N, true>> for IntExprNode<T, N, false> {
    type Error = IntError;
...
impl<T: VarLit, N: ArrayLength, const SIGN: bool> TryFrom<DynIntExprNode<T, SIGN>>
    for IntExprNode<T, N, SIGN>
{
    ...
    fn try_from(v: DynIntExprNode<T, SIGN>) -> Result<Self, Self::Error> {
        ...
        Ok(IntExprNode {
            creator: v.creator,
            indexes: GenericArray::try_from_slice(&v.indexes).unwrap().clone(),
        })
    }
}
