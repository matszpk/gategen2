    ...
    N: ArrayLength,
{
    fn bitmask(t: BoolExprNode<T>) -> Self {
        IntExprNode {
            ...
            indexes: GenericArray::try_from_iter(iter::repeat(t.index).take(N::USIZE)).unwrap(),
        }
    }
}
    ...
    ($pty:ty, $ty:ty, $($gparams:ident),*) => {
            ...
            $ty: ArrayLength,
        {
            fn constant(creator: Rc<RefCell<ExprCreator<T>>>, v: $pty) -> Self {
                IntExprNode{ creator, indexes: GenericArray::try_from_iter(
    ...
    ($pty:ty) => {
            ...
            N: ArrayLength,
        {
            fn try_constant(
                ...
                Ok(IntExprNode {
                    creator,
                    indexes: GenericArray::try_from_iter((0..n).into_iter().map(|x| {
                        // return 1 - true node index, 0 - false node index
                        if x < bits {
                            usize::from((v & (1 << x)) != 0)
                        } else {
